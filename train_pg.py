...
def sample_action(policy_net, state):
    probs = softmax(policy_net(state.unsqueeze(0)), dim=1)
...
def train_pg(policy_net, optimizer, asset_type, min_year=2006, max_year=2010,
    ...
    for i_epoch in range(num_epochs):
        ...
        logprobs_all = torch.zeros((n_steps, n_assets), dtype=torch.float32, device=device)
        ...
        for t in train_idx:
            ...
            # get actions and logprobs for each asset
            for i in range(n_assets):
                s = states[i]
                action, prob = sample_action(policy_net, s)
                actions_all[step, i] = action
                logprobs_all[step, i] = prob.log()
        ...
        for t in train_idx:
            ...
            policy_grad_op = discount_factor**step * g * logprobs
